...
import 'package:fitness_sync/common_widget/food_step_detail_row.dart';
...
class _FoodInfoDetailsViewState extends State<FoodInfoDetailsView> {
  ...
  List<Map<String, String>> createStepArr(Meals meal) {
    List<Map<String, String>> stepArr = [];

    for (int i = 0; i < meal.cookList.length; i++) {
      stepArr.add({
        "no": (i + 1).toString(),
        "detail": meal.cookList[i],
      });
    }
  ...
  List<Map<String, String>> createIngredientsArr(Meals meal) {
    final ingredientImagePaths = {
      "Wheat Flour": "assets/img/flour.png",
      "Sugar": "assets/img/sugar.png",
      "Baking Soda": "assets/img/baking_soda.png",
      "Eggs": "assets/img/eggs.png",
    };

    List<Map<String, String>> ingredientsArr = [];

    meal.ingredients.forEach((ingredient, value) {
      final imagePath =
          ingredientImagePaths[ingredient] ?? "assets/img/flour.png";
      ingredientsArr.add({
        "image": imagePath,
        "title": ingredient,
        "value": value,
  ...
  List<Map<String, String>> createNutritionArr(Meals meal) {
    final nutrientImagePaths = {
      Nutrients.carbohydrates: "assets/img/carbo.png",
      Nutrients.proteins: "assets/img/proteins.png",
      Nutrients.fats: "assets/img/fats.png",
      Nutrients.vitamins: "assets/img/vitamins.png", // Example path
      Nutrients.minerals: "assets/img/minerals.png", // Example path
      Nutrients.fibre: "assets/img/fibre.png", // Example path
      Nutrients.water: "assets/img/water.png", // Example path
    };
    ...
    // Add other nutrients
    meal.nutrients.forEach((nutrient, value) {
      final imagePath = nutrientImagePaths[nutrient] ?? "assets/img/water.png";
      final title = nutrient.customName;
      nutritionArr.add({
        "image": imagePath,
        "title": "${value}g $title",
      });
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      meal = widget.dObj["meal"];
      nutritionArr = createNutritionArr(meal!);
      ingredientsArr = createIngredientsArr(meal!);
      stepArr = createStepArr(meal!);
      setState(() {
        _isLoading = false;
      });
    });
