...
import 'package:fitness_sync/common_widget/find_eat_cell.dart';
import 'package:fitness_sync/common_widget/round_button.dart';
import 'package:fitness_sync/common_widget/today_meal_row.dart';
...
// "28/05/2023 08:00 AM"
  ...
  Future<void> getTodayMeal() async {
    await Provider.of<UserMealProvider>(context, listen: false)
        .fetchUserMeals(userId, DateTime.now());
    var userMeals =
        Provider.of<UserMealProvider>(context, listen: false).dailyMeals;
    todayMealArr = userMeals
        .where((value) => value.meal.mealType.customName == selectedValue)
        .map((userMeal) {
      return {
        "name": userMeal.meal.name,
        "image": userMeal.meal.icon,
        "time": userMeal.date,
        "nutritions": userMeal.meal.nutrients,
        "calories": userMeal.meal.calories
      };
    }).toList();
  }
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          gradient: LinearGradient(
                              colors: TColor
                                  .primaryG), // Replace with TColor.primaryG
                          borderRadius: BorderRadius.circular(15),
                        ),
                        child: DropdownButtonHideUnderline(
                          child: DropdownButton<String>(
                                ...
                                .map((name) => DropdownMenuItem<String>(
                                      ...
                                      child: Text(
                                        ...
                                        style: TextStyle(
                                            color: TColor.gray,
                                            fontSize:
                                                14), // Replace TColor.gray with Colors.grey
                                      ),
                                    ))
                                .toList(),
                            ...
                            icon: Icon(Icons.expand_more,
                                color: TColor
                                    .white), // Replace TColor.white with Colors.white
                            hint: Text(
                              selectedValue,
                              textAlign: TextAlign.center,
                              style: TextStyle(
                                  color: TColor.white,
                                  fontSize:
                                      12), // Replace TColor.white with Colors.white
                            ),
                          ),
                        ),
                      )
